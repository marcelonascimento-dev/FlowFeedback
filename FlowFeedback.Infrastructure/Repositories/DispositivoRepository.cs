...
public class DispositivoRepository(IDbConnectionFactory dbFactory) : IDispositivoRepository
{
    public async Task<Dispositivo?> GetByIdentifierAsync(string deviceIdentifier)
    {
        ...
        SELECT Id, EmpresaId, Nome, Identificador, Ativo, DataCriacao 
        FROM Dispositivos 
    ...
    {
        using var db = dbFactory.CreateTenantConnection();
        var sql = @"INSERT INTO Dispositivos (Id, EmpresaId, Nome, Identificador, Ativo, DataCriacao) 
                    VALUES (@Id, @EmpresaId, @Nome, @Identificador, @Ativo, @DataCriacao)";
